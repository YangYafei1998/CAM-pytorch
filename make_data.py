...
import os
import cv2
def generateListandLabel(path1, path2):
    # class_to_idx={}
    classes_camdrawer=[]
    # with open(f'{path1}/classes.txt') as f:
    #     for l in f:
    #         l=l[l.find(' ')+1:l.find('\n')]
    #         index=int(l[:l.find('.')])
    #         print("l is",l,", with index ",index-1)
    #         class_to_idx.update({l:index-1})
    #         classes_camdrawer.append(l)
    # print("class_to_idx:", class_to_idx)
    # print("classes to camdrawer", classes_camdrawer)
    ...
    # sorted_image_class_list = make_dataset(path, class_to_idx =class_to_idx, extensions='jpg,png')


    # with open("train_image_list_sorted.txt", "w") as f:
    #     for item in sorted_image_class_list:
    #         f.write(item[0])
    #         f.write("\n")

    # with open("train_image_label_sorted.txt", "w") as f:
    #     for item in sorted_image_class_list:
    #         f.write(str(item[1]))
    #         f.write("\n")
...
def seperateCUB(path='/userhome/30/jrwang/CUB_200_2011/'):
    # print(os.path.exists(f'{path}/image_class_labels.txt'))
    filenames=[f'{path}/train_test_split.txt',f'{path}/images.txt']
    if(not os.path.exists(f"{path}/test/")):
        os.makedirs(f"{path}/test/")
    if(not os.path.exists(f"{path}/train/")):
        os.makedirs(f"{path}/train/")
    
    with open(filenames[0]) as fp1, open(filenames[1]) as fp2:
        for l, p in zip(fp1, fp2):    
            label=l.rstrip()[l.find(' ')+1:]
            path1=p.rstrip()
            print("current path is ", path1)
            newdir=path1[path1.find(' ')+1 : path1.find('/')]
            print("current dir is ", newdir)
            print("current label is ", label)
            if int(label)==1:#train
                newdir=f"{path}/train/{newdir}"
                if(not os.path.exists(newdir)):
                    os.mkdir(newdir)
                path1=path1[path1.find(' ')+1:]
                print("TRAIN with label ", label, "path is ", path1)
                copyfile(f"{path}/images/{path1}", f"{path}/train/{path1}")
                # input()
            else:
                newdir=f"{path}/test/{newdir}"
                if(not os.path.exists(newdir)):
                    os.mkdir(newdir)
                path1=path1[path1.find(' ')+1:]
                print("TEST with label ", label, "path is ", path1)
                copyfile(f"{path}/images/{path1}", f"{path}/test/{path1}")

# seperateCUB()
generateListandLabel(path1='/userhome/30/jrwang/6Ctrain/images',path2='/userhome/30/jrwang/test/images' )